...
# """
# Contact Scraper Desktop UI – Multithreaded
# HIGH-DPI FRIENDLY (1920×1080) – All text enlarged
# """
# import tkinter as tk
# from tkinter import ttk, filedialog, messagebox, scrolledtext
# import threading
...
# 
# # ----------------------------------------------------------------------
# # HIGH-DPI FONT SCALING (1920×1080 friendly)
# # ----------------------------------------------------------------------
# _BASE = 10
# _SCALE = 1.35  # 35% larger — adjust if needed
# 
# def _sz(base: int) -> int:
#     return int(base * _SCALE)
# 
# # Pre-defined scaled fonts
# HEADER_TITLE_FONT   = ("Helvetica", _sz(20), "bold")
# HEADER_SUB_FONT     = ("Helvetica", _sz(12))
# LABEL_FONT          = ("Helvetica", _sz(12), "bold")
# ENTRY_FONT          = ("Helvetica", _sz(12))
# TREE_FONT           = ("Consolas",   _sz(13))
# LOG_FONT            = ("Consolas",   _sz(13))
# BUTTON_FONT         = ("Helvetica", _sz(11), "bold")
# STATUS_FONT         = ("Helvetica", _sz(10))
# 
# # Custom ttk styles for larger widgets
# def _configure_big_styles():
#     s = ttk.Style()
#     s.configure("Big.TButton", font=BUTTON_FONT)
#     s.configure("Big.TCheckbutton", font=("Helvetica", _sz(11)))
#     s.configure("Big.Treeview", font=TREE_FONT, rowheight=_sz(22))
#     s.configure("Big.Treeview.Heading", font=("Helvetica", _sz(11), "bold"))
# 
# _configure_big_styles()
# 
# # ----------------------------------------------------------------------
# # GUI
# # ----------------------------------------------------------------------
...
#     def __init__(self, root):
#         self.root = root
#         self.root.title("Contact Scraper - Email & Phone Extractor")
#         self.root.geometry("980x720")
#         self.root.minsize(800, 600)
# 
#         # style
...
# 
#         # --- Use scaled fonts ---
#         self.header_title_font = HEADER_TITLE_FONT
#         self.header_sub_font   = HEADER_SUB_FONT
...
#         tk.Label(
#             f, text="Contact Scraper", font=self.header_title_font,
#             bg=self.bg_color, fg="#2196F3"
#         ).pack()
#         tk.Label(
#             f, text="Extract emails & Nepali phone numbers (multithreaded)",
#             font=self.header_sub_font, bg=self.bg_color, fg="#666"
#         ).pack()
# 
#     def create_input_section(self, parent):
#         f = ttk.LabelFrame(parent, text="Configuration", padding=12)
#         f.grid(row=1, column=0, sticky="ew", pady=(0, 12))
#         f.columnconfigure(1, weight=1)
# 
#         # ---- mode ----
#         ttk.Label(f, text="Scraping Mode:", font=self.label_font).grid(
#             row=0, column=0, sticky="w", pady=6
#         )
#         mode_f = ttk.Frame(f)
#         mode_f.grid(row=0, column=1, sticky="w", pady=6)
#         for txt, val in [
#             ("Single URL", "url"),
#             ("Google Maps Search", "keywords"),
#             ("File (URLs)", "file"),
...
# 
#         # ---- URL ----
#         self.url_lbl = ttk.Label(f, text="Website URL:", font=self.label_font)
#         self.url_ent = ttk.Entry(f, textvariable=self.url_var, width=55, font=self.entry_font)
# 
#         # ---- Keywords ----
#         self.kw_lbl = ttk.Label(f, text="Search Keywords:", font=self.label_font)
#         self.kw_ent = ttk.Entry(f, textvariable=self.keywords_var, width=55, font=self.entry_font)
# 
#         # ---- Number of sites ----
#         self.num_lbl = ttk.Label(f, text="Number of Sites:", font=self.label_font)
#         self.num_spn = ttk.Spinbox(
#             f, from_=1, to=50, textvariable=self.num_sites_var, width=10, font=self.entry_font
#         )
# 
#         # ---- File ----
#         self.file_lbl = ttk.Label(f, text="File with URLs:", font=self.label_font)
#         self.file_fr = ttk.Frame(f)
#         self.file_ent = ttk.Entry(self.file_fr, textvariable=self.file_path_var,
#                                  state="readonly", width=45, font=self.entry_font)
#         self.file_ent.pack(side="left", fill="x", expand=True)
#         ttk.Button(self.file_fr, text="Browse...", command=self.browse_file,
#                    style="Big.TButton").pack(side="left", padx=(6, 0))
# 
#         # ---- Workers ----
#         self.workers_lbl = ttk.Label(f, text="Max Threads:", font=self.label_font)
#         self.workers_spn = ttk.Spinbox(
#             f, from_=1, to=30, textvariable=self.max_workers_var, width=10, font=self.entry_font
#         )
# 
#         # ---- Save checkbox ----
#         ttk.Checkbutton(
...
#         ).grid(row=99, column=0, columnspan=2, sticky="w", pady=12)
# 
#         self.on_mode_change()  # initial visibility
# 
#     def create_log_section(self, parent):
#         f = ttk.LabelFrame(parent, text="Logs & Results", padding=12)
#         f.grid(row=2, column=0, sticky="nsew", pady=(0, 12))
#         f.columnconfigure(0, weight=1)
#         f.rowconfigure(0, weight=1)
# 
#         nb = ttk.Notebook(f)
...
#         nb.add(log_tab, text="Console Logs")
#         self.log_txt = scrolledtext.ScrolledText(
#             log_tab, wrap="word", font=self.log_font,
#             bg="#1e1e1e", fg="#d4d4d4", insertbackground="white"
#         )
#         self.log_txt.pack(fill="both", expand=True)
# 
...
#         self.tree.column("Emails", width=350)
#         self.tree.column("Phones", width=300)
# 
#         vsb.pack(side="right", fill="y")
#         hsb.pack(side="bottom", fill="x")
#         self.tree.pack(side="left", fill="both", expand=True)
...
#             padx=24, pady=10, relief="flat", cursor="hand2", state="disabled"
#         )
#         self.stop_btn.pack(side="left", padx=6)
# 
#         tk.Button(
#             f, text="Clear Logs", command=self.clear_logs,
#             bg="#757575", fg="white", font=self.button_font,
...
# 
#         self.status_lbl = tk.Label(
#             f, text="Ready", font=self.status_font,
#             bg=self.bg_color, fg="#666"
#         )
#         self.status_lbl.pack(side="right", padx=12)
# 
#         self.progress = ttk.Progressbar(f, mode="determinate")
#         self.progress.pack(side="right", fill="x", expand=True, padx=12)
# 
#     # ------------------------------------------------------------------
#     # UI helpers
#     # ------------------------------------------------------------------
...
# 
#         if mode == "url":
#             self.url_lbl.grid(row=1, column=0, sticky="w", pady=6)
#             self.url_ent.grid(row=1, column=1, sticky="ew", pady=6, padx=6)
#         elif mode == "keywords":
#             self.kw_lbl.grid(row=1, column=0, sticky="w", pady=6)
#             self.kw_ent.grid(row=1, column=1, sticky="ew", pady=6, padx=6)
#             self.num_lbl.grid(row=2, column=0, sticky="w", pady=6)
#             self.num_spn.grid(row=2, column=1, sticky="w", pady=6, padx=6)
#             self.workers_lbl.grid(row=3, column=0, sticky="w", pady=6)
#             self.workers_spn.grid(row=3, column=1, sticky="w", pady=6, padx=6)
#         elif mode == "file":
#             self.file_lbl.grid(row=1, column=0, sticky="w", pady=6)
#             self.file_fr.grid(row=1, column=1, sticky="ew", pady=6, padx=6)
#             self.workers_lbl.grid(row=2, column=0, sticky="w", pady=6)
#             self.workers_spn.grid(row=2, column=1, sticky="w", pady=6, padx=6)
# 
#     def browse_file(self):
#         path = filedialog.askopenfilename(
#             title="Select file with URLs",
...
#                 return
# 
#             # ---- 2. ThreadPoolExecutor for THIS run --------------------
#             self.executor = ThreadPoolExecutor(max_workers=max_workers)
#             self.futures = {
#                 self.executor.submit(scrape_one_site, url): url for url in sites
#             }
